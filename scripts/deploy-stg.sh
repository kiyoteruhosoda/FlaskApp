...
if [ ! -f "$ENV_FILE" ]; then
  echo -e "\033[33m[deploy-stg][warn] $ENV_FILE not found; generating a default template.\033[0m"
  ...
  mkdir -p "$BASE_DIR"
  cat > "$ENV_FILE" <<ENVEOF
# 自動生成された .env（deploy-stg スクリプトが作成）。
# 資格情報などは docker-compose.yml の既定値で起動する（初期設定のみで動作）。
# 既定の資格情報は開発向け。外部公開する場合は必ず上書きして再デプロイする。
# すべての項目は .env.staging.example を参照。

# --- STG 固有の実値（compose 既定値は本番向けのため、ここで STG 用に固定する）---
...
WEB_HOST_PORT=8051
DB_HOST_PORT=3308
DB_CONTAINER_NAME=mariadb-stg
DOCKER_NETWORK_NAME=photonest-stg
...
# API_BASE_URL=https://stg.photonest.example.com
# CORS_ALLOWED_ORIGINS=https://stg.photonest.example.com
