...
| セキュリティ / 署名 | `SECRET_KEY`, `ACCESS_TOKEN_ISSUER`, `ACCESS_TOKEN_AUDIENCE`, `SERVICE_ACCOUNT_SIGNING_AUDIENCE`, `ENCRYPTION_KEY`, `ENCRYPTION_KEY_FILE`, `WEBAUTHN_RP_ID`, `WEBAUTHN_ORIGIN`, `WEBAUTHN_RP_NAME` | セッション署名・トークン発行・WebAuthn 認証情報に利用 |
