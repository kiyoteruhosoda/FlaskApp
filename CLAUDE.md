...
| 項目          | 指針                                                                                       |
| ----------- | ---------------------------------------------------------------------------------------- |
| **定義場所**    | `shared/kernel/settings/settings.py` に `class 用途名Settings`を定義。                                  |
| **アクセス経路**  | すべてのコードは `from shared.kernel.settings.settings import settings` をインポートし、<br>**`settings.py` プロパティ経由で取得**。 |
...
* 新しい設定キーを追加する場合は必ず以下を更新：
  ...
  * `shared/kernel/settings/system_settings_defaults.py`（デフォルト値）
  * `shared/kernel/settings/settings.py`（`@property` を追加）
  * `presentation/web/admin/system_settings_definitions.py`（管理画面項目）
