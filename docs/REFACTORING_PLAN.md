...
正本へ未張替で、後方互換シム経由で動いている consumer:

| 層 | core.* を参照するファイル数（概数） |
|---|---|
| `tests/` | 約 93 |
| `presentation/` | 約 28 |
| `cli/` | 3 |
...
### 2.5 既知の先在バグ（本リファクタとは無関係）

- `bounded_contexts/email/infrastructure/smtp_sender.py` が
...
### Phase A — consumer の core.* 参照を正本へ張替

`core/` シムをまだ参照している層を正本へ張り替える（§2.2 の対応表を使用）。

1. **本番系**: `presentation/`・`cli/`・`scripts/`・`main.py`・`wsgi.py`。
   - 例: `from core.db import db` → `from shared.kernel.database.db import db`。
   - フレーズ単位置換で `core.db_log_handler` 等の誤爆を避ける（長いキーを先に置換）。
2. **`tests/`**: 約 93 ファイル。数が多いため本番系とは別 PR を推奨。
   - 一部テストはシムの再エクスポート同一性を検証している（`tests/unit/core/`）。
     シム削除（Phase B）時に整理する前提で、まずは正本参照へ張替。
3. **`migrations/`**: `migrations/versions/8c1f2e3d4b5a_*.py` が
   `core.system_settings_defaults` を参照。マイグレーションは履歴として基本不変だが、
   import パスのみの更新は安全（正本へ張替してシム削除に備える）。

完了条件: `grep -rn "from core\.\|import core\b" --include='*.py' .`（`core/` 除く）が
`core.version` 関連を除きゼロ。
...
`bounded_contexts/email/infrastructure/smtp_sender.py` の `email_message` import を
`bounded_contexts.email_sender.domain.email_message` へ修正（§2.5）。
