...
album_item = db.Table(
    "album_item",
    db.Column("album_id", db.BigInteger, db.ForeignKey("album.id"), primary_key=True),
    db.Column("media_id", db.BigInteger, db.ForeignKey("media.id"), primary_key=True),
    db.Column("sort_index", db.BigInteger),
)
...
class Media(db.Model):
    ...
    account_id = db.Column(db.BigInteger, db.ForeignKey('google_account.id'), nullable=False)
    ...
    live_group_id = db.Column(db.BigInteger)
...
class MediaSidecar(db.Model):
    id = db.Column(db.BigInteger, primary_key=True)
    media_id = db.Column(db.BigInteger, db.ForeignKey('media.id'), nullable=False)
    type = db.Column(db.Enum('video', 'audio', 'subtitle', name='media_sidecar_type'), nullable=False)
    rel_path = db.Column(db.String(255))
    bytes = db.Column(db.BigInteger)

class Exif(db.Model):
    media_id = db.Column(db.BigInteger, db.ForeignKey('media.id'), primary_key=True)
...
class Album(db.Model):
    ...
    cover_media_id = db.Column(db.BigInteger, db.ForeignKey('media.id'))
