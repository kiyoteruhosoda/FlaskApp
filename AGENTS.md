...
この形式で、リポジトリのルートに `AGENTS.md` を配置すれば、
新規メンバーへの開発ポリシー・設計・環境ルールが一目でわかる構成になります。

ご希望があれば、Alembicマイグレーション用のテンプレート（共通ヘッダ＋コメント付き）も併記しますか？



本ドキュメントは、開発における共通ルールと設計指針を定義します。
目的は **重複ロジックの排除**、**可読性・保守性の確保**、**責務分離**、および **安定したテスト** の実現です。
