...
from shared.kernel.version import (
    VERSION_FILE_PATH,
...
class TestVersionCore:
    ...
    def test_get_version_info_with_file(self):
        ...
        with patch("shared.kernel.version.load_version_from_file", return_value=mock_version_data):
            result = get_version_info()

        assert result["version"] == "v1a2b3c4"
        assert result["commit_hash"] == "1a2b3c4"
        assert result["branch"] == "main"
        assert "app_start_date" in result

    def test_get_version_info_without_file(self):
        """バージョンファイルがない場合のテスト"""
        with patch("shared.kernel.version.load_version_from_file", return_value=None):
            result = get_version_info()
...
class TestVersionIntegration:
    ...
    def test_real_version_file_creation_and_reading(self):
        ...
        try:
            with patch("shared.kernel.version.VERSION_FILE_PATH", temp_path):
                result = load_version_from_file()
    ...
    def test_version_file_path_exists(self):
        ...
        assert VERSION_FILE_PATH.endswith("shared/kernel/version.json")
        assert "core/version.py" not in VERSION_FILE_PATH
