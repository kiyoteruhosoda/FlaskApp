...
| 優先 | # | 概要 | 状態 | 影響度 | 工数 |
|---|---|---|---|---|---|
| 1 | T13 | デプロイ後の Auth 失効で画像が出ず、強制ログアウトもされない | ⬜未着手 | 中 | 中 |
| 2 | T16 | システム画像フォーマットを AVIF にする（画像変換を AVIF 出力へ） | ⬜未着手 | 中 | 中 |
| 3 | T3 | 初回ログイン時パスワード強制変更フローの配線（既定 OFF） | 🚧進行中 | 小 | 中 |
| 4 | T4 | `bounded_contexts/email` と `bounded_contexts/email_sender` の重複統合 | 🟡要判断 | 中 | 中 |
| 5 | T5 | Photo Exports（/admin/photo-exports）画面が未実装（プレースホルダーのみ） | ⬜未着手 | 小 | 中 |
| 6 | T7 | ユーザーとグループの紐づけ画面 | ⬜未着手 | 中 | 中 |
| 7 | T8 | グループとロールの紐づけ | ⬜未着手 | 中 | 中 |
| 8 | T9 | ユーザースイッチ（運用管理者ロールによる成り代わり） | ⬜未着手 | 中 | 大 |
| 9 | T10 | ロール設計見直し（運用管理者・運用者・システムオーナーの分離） | 🟡要判断 | 大 | 中 |
| 10 | T11 | FastAPI 移行 | 🟡要判断 | 大 | 大 |

---

## 詳細

- **T13 デプロイ後の Auth 失効** — ログインしたままデプロイ（サーバー再起動）すると画像が
  表示されなくなる。サムネイルは `POST /media/{id}/thumb-url` で得た署名 URL を `<img src>` で
  直接読み込むため（`frontend/src/pages/MediaPage.tsx`）、この img リクエストの 401 は axios の
  レスポンスインターセプター（`frontend/src/services/api.ts`）を通らず、`forceLogout` が発火せず
  画面が強制ログアウトされない。要再現・原因特定のうえ、失効検知→ログイン画面遷移を配線する。
- **T16 画像フォーマット AVIF 化** — サムネイル生成
  （`bounded_contexts/photonest/tasks/thumbs_generate.py`）は現状 JPEG/PNG 出力。システムで扱う
  画像フォーマットを AVIF に統一する。Pillow の AVIF opener 登録（`pillow-heif`）、出力拡張子・
  保存処理、配信側の Content-Type、既存サムネイルの扱いまで影響するため要設計。
