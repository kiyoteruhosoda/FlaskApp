...
def _setup_admin_with_directories(client, tmp_path):
    ...
    app.config["MEDIA_THUMBNAILS_DIRECTORY"] = str(thumbs_dir)
    app.config["MEDIA_PLAYBACK_DIRECTORY"] = str(playback_dir)
...
def test_data_files_allows_switching_filtering_and_pagination(client, tmp_path):
    ...
    response = client.get("/admin/data-files?directory=MEDIA_PLAYBACK_DIRECTORY")
    assert response.status_code == 200
    html = response.data.decode("utf-8")
    assert "photo1.mp4" in html
    assert "photo1.jpg" not in html

    response = client.get("/admin/data-files?directory=MEDIA_ORIGINALS_DIRECTORY&per_page=1")
    html = response.data.decode("utf-8")
    assert html.count("photo1.jpg") == 1
    assert "photo2.jpg" not in html
    assert "page-link" in html

    response = client.get("/admin/data-files?directory=MEDIA_ORIGINALS_DIRECTORY&per_page=1&page=2")
    html = response.data.decode("utf-8")
    assert "photo2.jpg" in html
    assert "photo1.jpg" not in html

    response = client.get("/admin/data-files?directory=MEDIA_ORIGINALS_DIRECTORY&q=photo2")
    html = response.data.decode("utf-8")
    assert "photo2.jpg" in html
    assert "photo1.jpg" not in html

    response = client.get("/admin/data-files?directory=MEDIA_ORIGINALS_DIRECTORY&q=nope")
    html = response.data.decode("utf-8")
