...
|---|---|---|---|
| `docs/OPERATIONS.md` | 手順書 | 「〇〇したいとき、〇〇する」という操作手順・コマンドのみ | なぜそうなっているか、過去に何が起きたか、内部の仕組み |
...
迷ったら「これは手順か（OPERATIONS）」「現在の仕様か（README）」
「過去の経緯か（CHANGELOG）」で判断する。同じ内容を複数箇所に重複して書かない。
