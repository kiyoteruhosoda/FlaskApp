...
@bp.post("/api/google/oauth/start")
@login_required
def google_oauth_start():
    ...
    params = {
        ...
        "redirect_uri": url_for("photo_view.google_oauth_callback", _external=True),
        "response_type": "code",
        "scope": " ".join(scopes),
        "access_type": "offline",
...
def google_oauth_callback():
    ...
    if request.args.get("error"):
        ...
        return redirect(url_for("photo_view.google_accounts"))
    ...
    try:
        ...
        flash(_("Failed to obtain token from Google."), "error")
        return redirect(url_for("photo_view.google_accounts"))
    ...
    redirect_to = saved.get("redirect") or url_for("photo_view.google_accounts")
    session.pop("google_oauth_state", None)
    flash(_("Google account linked: %(email)s", email=email), "success")
    return redirect(redirect_to)


@bp.get("/api/google/accounts")
@login_required
