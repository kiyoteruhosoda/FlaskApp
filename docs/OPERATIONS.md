...
Synology / Docker 環境では通常、各環境ディレクトリ（`photonest/prod` / `photonest/stg`）で
`./scripts/deploy.sh migrate` から自動適用する（3. デプロイ参照）。
...
#   （環境ディレクトリ photonest/prod または photonest/stg で実行する）
...
```bash
# 環境ディレクトリ（photonest/prod または photonest/stg）で実行する。
# deploy.sh は配置ディレクトリ名から環境を自動判定する（stg / prod 共通スクリプト）。
./scripts/deploy.sh app       # アプリのみ更新
./scripts/deploy.sh migrate   # DDL更新
./scripts/deploy.sh reset     # 完全初期化
```

デプロイ中にエラーが発生した場合は、失敗したモジュール（コンテナ）のログが
自動で出力されて終了する（追加のコマンド実行は不要）。
...
```bash
# 本番
cd /volume1/docker/photonest/prod
docker compose -p photonest logs web --tail 100
docker compose -p photonest logs worker --tail 50

# STG
cd /volume1/docker/photonest/stg
docker compose -p photonest-stg logs web --tail 100
```

> 別ディレクトリから実行する場合は `docker compose -p photonest -f /volume1/docker/photonest/prod/docker-compose.yml --env-file /volume1/docker/photonest/prod/.env logs web` のように `-p`/`-f`/`--env-file` を明示する。
...
ディレクトリ配置・成果物の転送先は `scripts/README.md`（`deploy.sh`）参照。

**1. .env 設定**（`photonest/<stg|prod>/.env`。最低限変更が必要な項目）
...
```bash
# 開発マシンで（成果物は dist/ に出力される）
./scripts/.build.sh
scp dist/image.tar dist/image-db.tar <user>@<nas-ip>:/volume1/docker/photonest/prod/
scp dist/scripts/deploy.sh <user>@<nas-ip>:/volume1/docker/photonest/prod/scripts/
# （NAS 側に pick.sh を用意している場合はそれで dist/ から取得してもよい）
...
| 症状 | 対処 |
|---|---|
| `deploy.sh` が `permission denied ... docker.sock` で失敗する | `sudo` を付けて実行するか、実行ユーザーを `docker` グループに追加して再ログインする |
...
| `dist/image.tar` が異常に大きい | `docker images photonest:latest` / `docker history photonest:latest --no-trunc` でレイヤーを確認する |
