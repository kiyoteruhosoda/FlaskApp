...
UI は **React SPA のみ**（Jinja 完全廃止済）：

- **React SPA** — `frontend/`（Vite + React + Redux + react-router）。
  ルートに**未マッチの非API**は React の catch-all が SPA を返す。

**全 Jinja テンプレート削除済み**（email テンプレート・OpenAPI UI を除く）。
Flask 側は各 Jinja ルートを対応する React パスへリダイレクト、または JSON API のみ。

> 凡例（React 状況）: ✅=React実装済 / 🟡=部分 / ⬜=未着手
...
12. ✅ Jinja からの完全切替（auth・admin・photo-view・certs・TOTP の Jinja テンプレート撤去・render_template 全廃・email/OpenAPI テンプレートのみ残存）
