...
``bounded_contexts.storage.domain`` に定義されていた列挙型を ``shared.kernel`` へ昇格させた。
``shared`` レイヤーが bounded context を import しなくても型を参照できるようにする目的。
``bounded_contexts.storage.domain.types`` はこのモジュールから再エクスポートする。
"""
