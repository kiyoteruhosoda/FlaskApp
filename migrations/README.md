...
- マスタデータ（ロール・権限・管理者ユーザー・システム設定）は**含めない**。
  スキーマ作成後に冪等な `python scripts/seed_master_data.py` で投入する。

### 新規 DB の初期化

```bash
flask db upgrade            # 全テーブル作成
python scripts/seed_master_data.py   # マスタデータ投入（冪等）
```
