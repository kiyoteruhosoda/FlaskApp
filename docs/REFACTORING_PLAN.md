...
- **`core/storage/` + `core/storage_service.py` → storage context ✅**:
  `core/storage`（パス解決サービス `StorageService`/`StorageArea` 系）を
  `bounded_contexts/storage/infrastructure/filesystem/` へ移設（シム残置）。
  これは `bounded_contexts/storage` の object-backend 抽象（`StorageBackend`/
  `StorageApplicationService`）とは**別の関心事**（パス解決 vs バイト read/write）で
  あり、1クラス化ではなく所有 context への配置として整理した。photonest の
  consumer は canonical 参照に更新。`shared.settings` の遅延 factory と presentation
  は引き続きシム経由（shared→context の逆方向結合を避けるため）。
- **`core/version.py`（据え置き）**: `core/version.json` 生成成果物と密結合のため
  移設価値が低く、`core/` に残す。

**4-f. contexts/shared の core シム依存の一掃 ✅**
Phase 4-c のタスク移設時、移設ファイル内の `core.tasks` 以外のシム参照
（`core.db` / `core.settings` / `core.models.*` / `core.logging_config` / `core.crypto`
等）が残っていた。`bounded_contexts/` ・ `shared/` 内の全 `core.*` 参照を正本へ張替。
→ **context／shared から core シムへの依存は完全にゼロ。**

> 残置している `core/` の非シム実体は `core/version.py` のみ。それ以外は
> `models/__init__`・`tasks/__init__`・各種シム群（db/settings/storage 等）で、
> 実装本体はすべて context／shared へ移動済み。
...
- [x] Phase 4-e: `core/storage` を storage context（`infrastructure/filesystem`）へ移設
- [x] Phase 4-f: contexts/shared の残存 core シム依存を一掃（core 依存ゼロ）
